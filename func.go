...
func panicf(v string, a ...any) { panic(fmt.Sprintf(v, a...)) }
	...
	for i := 0; i < numInDst; i++ {
		...
		if isSrcVariadic && srcIdx == numInSrc-1 && !(isDstVariadic && i == numInDst-1) {
			...
			steps = append(steps, func(inVal reflect.Value, callIn *[]reflect.Value) {
				val, ok := conv(inVal)
				if !ok {
					panic(errors.Wrapf(ErrTypeMismatch, "could not convert %T [%d]: (%v) to %v", inVal.Interface(), idx, inVal, variadicType))
				}
		...
		if isDstVariadic && i == numInDst-1 {
			...
			if isSrcVariadic && srcIdx == numInSrc-1 {
				...
				steps = append(steps, func(inVal reflect.Value, callIn *[]reflect.Value) {
					val, ok := conv(inVal)
					if !ok {
						panic(errors.Wrapf(ErrTypeMismatch, "could not convert %T (%v) to %v", inVal.Interface(), inVal, castType))
					}
					*callIn = append(*callIn, val)
				})
			} else {
				...
				steps = append(steps, func(inVal reflect.Value, callIn *[]reflect.Value) {
					...
					if isSrcVariadic {
						if inLen < len(discreteConverters) {
							panic(errors.Wrapf(ErrArgCount, "function must have the same number of arguments as the output function (%v), got %v", numInDst, inLen))
						}
					} else {
						if inLen != len(discreteConverters) {
							panic(errors.Wrapf(ErrArgCount, "function must have the same number of arguments as the output function (%v), got %v", numInDst, inLen))
					...
					for j, conv := range discreteConverters {
						...
						val, ok := conv(elem)
						if !ok {
							panic(errors.Wrapf(ErrTypeMismatch, "could not convert %T [%d]: (%v) to %v", elem.Interface(), j, elem, srcTypes[srcIdx+j]))
						}
						*callIn = append(*callIn, val)
					}
					...
					if isSrcVariadic {
						for j := len(discreteConverters); j < inLen; j++ {
							...
							if !ok {
								panic(errors.Wrapf(ErrTypeMismatch, "could not convert %T [%d]: (%v) to %v", elem.Interface(), j, elem, variadicDstType))
							}
							*callIn = append(*callIn, val)
						}
					}
		...
		steps = append(steps, func(inVal reflect.Value, callIn *[]reflect.Value) {
			val, ok := conv(inVal)
			if !ok {
				panic(errors.Wrapf(ErrTypeMismatch, "could not convert %T (%v) to %v", inVal.Interface(), inVal, typ))
			}
			*callIn = append(*callIn, val)
		})
		srcIdx++
...
func compileReturnConverter(srcFnTyp, dstFnTyp reflect.Type) (func([]reflect.Value) []reflect.Value, error) {
	...
	return func(res []reflect.Value) []reflect.Value {
		...
		default:
			panic(errors.Wrapf(ErrReturnCount, "function must return the same number of values as the output function (%v), got %v", numOutDst, numOutSrc))
		}

		for i, curr := range res {
			var cnvrted, ok = retConverters[i](curr)
			if !ok {
				panic(errors.Wrapf(ErrReturnCount, "function return value %v is not convertible to %v", curr.Type(), dstTypes[i]))
			}
			res[i] = cnvrted
		}
		return res
...
func (c *Func) AdheresTo(fn any) bool {
	...
	if fnType.Kind() != reflect.Func {
		panicf("fn must be a function, got %s", fnType.Kind())
	}
...
func (c *Func) Requires(index int, typ reflect.Type) *Func {
	...
	if c.Type.In(index) != typ && !c.Type.In(index).ConvertibleTo(typ) && !(typ.Kind() == reflect.Interface && c.Type.In(index).Implements(typ)) {
		panicf(
...
func (c *Func) Returns(returns ...reflect.Type) *Func {
	if c.Type.NumOut() != len(returns) {
		panic("function must return the same number of values as the number of types passed to Returns")
	}

	for i, typ := range returns {
		if c.Type.Out(i) != typ && !c.Type.Out(i).ConvertibleTo(typ) && !(typ.Kind() == reflect.Interface && c.Type.Out(i).Implements(typ)) {
			panicf(
...
func (c *Func) CallFunc(in []reflect.Value) []interface{} {
	...
	if c.BeforeExec != nil {
		...
		if err != nil {
			panicf("BeforeExec function returned an error: %v", err)
		}
	}

	for i, typ := range c.requiresIn {
		if in[i].Type() != typ && !in[i].Type().ConvertibleTo(typ) && !(typ.Kind() == reflect.Interface && in[i].Type().Implements(typ)) {
			panicf(
	...
	for i, curr := range out {
		...
		if currType != typ && !currType.ConvertibleTo(typ) && !(typ.Kind() == reflect.Interface && currType.Implements(typ)) {
			panicf(
				"function return value %v is not convertible to %v",
				currType, typ,
			)
...
func (c *Func) Call(args ...interface{}) []interface{} {
	...
	if c.Type.IsVariadic() {
		...
		if c.Type.NumIn() != len(args) {
			panicf(
				"function %T must have the same number of arguments as the number of arguments passed to Call (%v), got %v: %s",
